...
#SBATCH --output=/mnt/weka/akhosrovyan/logs_geocrossbench/tmlr_x-sen1floods11_dinov3_s2_head_%j.log
#SBATCH --array=0-23
...
torchrun \
  ...
  $seed \
  --experiment_name \
  TMLR_x-sen1floods11_dinov3_s2_head_seed${seed}_bs8_ep100_lr${lr}_uw${upernet_width} \
  --dataset_name \
  sen1floods11 \
  --dataset_path \
...
python \
  ...
  --checkpoint_path \
  /nfs/h100/raid/rs/ckpt_rs_finetune/segmentation/TMLR_x-sen1floods11_dinov3_s2_head_seed${seed}_bs8_ep100_lr${lr}_uw${upernet_width}/best_model.pth \
  --size \
  224 \
  --classes \
  2 \
