#!/bin/bash -l
#SBATCH --job-name=tmlr_sen1floo_croma_rgb_h
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=16
#SBATCH --mem=32G
#SBATCH --time=50:00:00
#SBATCH --partition=all
#SBATCH --output=/mnt/weka/akhosrovyan/logs_geocrossbench/tmlr_x-sen1floods11_croma_rgb_head_%j.log
#SBATCH --array=0-23
...
torchrun \
  ...
  $seed \
  --experiment_name \
  TMLR_x-sen1floods11_croma_rgb_head_seed${seed}_bs8_ep100_lr${lr}_uw${upernet_width} \
  --dataset_name \
  sen1floods11 \
  --dataset_path \
...
python \
  ...
  --checkpoint_path \
  /nfs/h100/raid/rs/ckpt_rs_finetune/segmentation/TMLR_x-sen1floods11_croma_rgb_head_seed${seed}_bs8_ep100_lr${lr}_uw${upernet_width}/best_model.pth \
  --size \
  120 \
  --classes \
  2 \
  --filename \
  logs_ICLR/seg/TMLR_x-sen1floods11_croma_rgb_head_ \
  --upernet_width \
  $upernet_width \
  --bands \
