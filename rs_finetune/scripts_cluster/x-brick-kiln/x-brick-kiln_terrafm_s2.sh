#!/bin/bash -l
#SBATCH --job-name=tmlr_brick-ki_terrafm_s2_f
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=16
#SBATCH --mem=32G
#SBATCH --time=50:00:00
#SBATCH --partition=all
#SBATCH --output=/mnt/weka/akhosrovyan/logs_geocrossbench/tmlr_x-brick-kiln_terrafm_s2_full_%j.log
#SBATCH --array=0-7
...
python \
  train_classifier.py \
  --experiment_name \
  brick-kiln/x-brick-kiln_terrafm_s2/seed${seed}_bs64_ep50_lr${lr} \
  --dataset_name \
  m_brick \
  --in_features \
  768 \
...
python \
  ...
  --checkpoint_path \
  /mnt/weka/akhosrovyan/ckpt_rs_finetune/classification/brick-kiln/x-brick-kiln_terrafm_s2/seed${seed}_bs64_ep50_lr${lr}/best-model.ckpt \
  --img_size \
  224 \
  --filename \
