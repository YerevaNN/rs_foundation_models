...
#SBATCH --output=/mnt/weka/akhosrovyan/logs_geocrossbench/tmlr_x-so2sat_dinov2_s2_full_%j.log
#SBATCH --array=0-7
...
python \
  train_classifier.py \
  --experiment_name \
  so2sat/x-so2sat_dinov2_s2/seed${seed}_bs64_ep50_lr${lr} \
  --dataset_name \
  so2sat \
  --in_features \
  768 \
  ...
  --checkpoint_path \
  /mnt/weka/akhosrovyan/ckpt_rs_finetune/classification/so2sat/x-so2sat_dinov2_s2 \
...
python \
  ...
  ./configs/so2sat.json \
  --checkpoint_path \
  /mnt/weka/akhosrovyan/ckpt_rs_finetune/classification/so2sat/x-so2sat_dinov2_s2/seed${seed}_bs64_ep50_lr${lr}/best-model.ckpt \
  --img_size \
  224 \
  --filename \
  /mnt/weka/akhosrovyan/logs_geocrossbench/cls/TMLR_x-so2sat_dinov2_s2_full_ \
  --bands \
