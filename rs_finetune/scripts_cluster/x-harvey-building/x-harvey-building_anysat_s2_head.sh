#!/bin/bash -l
#SBATCH --job-name=tmlr_harvey-b_anysat_s2_h
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=16
#SBATCH --mem=32G
#SBATCH --time=50:00:00
#SBATCH --partition=all
#SBATCH --output=/mnt/weka/akhosrovyan/logs_geocrossbench/tmlr_x-harvey-building_anysat_s2_head_%j.log
#SBATCH --array=0-23
...
torchrun \
  ...
  $seed \
  --experiment_name \
  TMLR_x-harvey-building_anysat_s2_head_seed${seed}_bs8_ep100_lr${lr}_uw${upernet_width} \
  --dataset_name \
  harvey \
  --dataset_path \
...
python \
  ...
  ./configs/harvey.json \
  --checkpoint_path \
  /nfs/h100/raid/rs/ckpt_rs_finetune/segmentation/TMLR_x-harvey-building_anysat_s2_head_seed${seed}_bs8_ep100_lr${lr}_uw${upernet_width}/best_model.pth \
  --size \
  224 \
  --classes \
  2 \
  --filename \
  logs_ICLR/seg/TMLR_x-harvey-building_anysat_s2_head_ \
  --upernet_width \
  $upernet_width \
  --bands \
