#!/bin/bash -l
#SBATCH --job-name=tmlr_bigearth_dinov3_s2_f
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=16
#SBATCH --mem=32G
#SBATCH --time=50:00:00
#SBATCH --partition=all
#SBATCH --output=/mnt/weka/akhosrovyan/logs_geocrossbench/tmlr_x-bigearthnet_dinov3_s2_full_%j.log
#SBATCH --array=0-7
...
python \
  train_classifier.py \
  --experiment_name \
  bigearthnet/x-bigearthnet_dinov3_s2/seed${seed}_bs64_ep50_lr${lr} \
  --dataset_name \
  m_ben \
  --in_features \
  768 \
  ...
  --checkpoint_path \
  /mnt/weka/akhosrovyan/ckpt_rs_finetune/classification/bigearthnet/x-bigearthnet_dinov3_s2 \
...
python \
  ...
  ./configs/m_ben.json \
  --checkpoint_path \
  /mnt/weka/akhosrovyan/ckpt_rs_finetune/classification/bigearthnet/x-bigearthnet_dinov3_s2/seed${seed}_bs64_ep50_lr${lr}/best-model-f1.ckpt \
  --img_size \
  224 \
  --filename \
