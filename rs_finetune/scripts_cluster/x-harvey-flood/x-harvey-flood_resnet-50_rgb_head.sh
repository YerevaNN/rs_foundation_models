#!/bin/bash -l
#SBATCH --job-name=tmlr_harvey-f_resnet50_rgb_h
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=16
#SBATCH --mem=32G
#SBATCH --time=50:00:00
#SBATCH --partition=all
#SBATCH --output=/mnt/weka/akhosrovyan/logs_geocrossbench/tmlr_x-harvey-flood_resnet-50_rgb_head_%j.log
#SBATCH --array=0-23
...
torchrun \
  ...
  train_change.py \
  --experiment_name \
  TMLR_x-harvey-flood_resnet-50_rgb_head_seed${seed}_bs8_ep100_lr${lr}_uw${upernet_width} \
  --mode \
  vanilla \
  --dataset_name \
  harvey \
...
python \
  ...
  ./configs/harvey.json \
  --checkpoint_path \
  /nfs/h100/raid/rs/ckpt_rs_finetune/change_detection/TMLR_x-harvey-flood_resnet-50_rgb_head_seed${seed}_bs8_ep100_lr${lr}_uw${upernet_width}/best_model.pth \
  --size \
  224 \
  --filename \
  logs_ICLR/cd/TMLR_x-harvey-flood_resnet-50_rgb_head_ \
  --bands \
