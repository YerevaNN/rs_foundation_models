...
class ChannelVisionTransformer(nn.Module):
    ...
    def prepare_tokens(self, x, extra_tokens):
        B, nc, w, h = x.shape
        x, nc, x_shape = self.patch_embed(x, extra_tokens)  # patch linear embedding
        out_size = (nc, x_shape[-2], x_shape[-1])
    ...
    def forward(self, x, extra_tokens={}):
        x, hw_shape = self.prepare_tokens(x, extra_tokens=extra_tokens)
...
cvit_encoders = {
    "Cvit-B": {
        "encoder": ChannelVisionTransformer,
