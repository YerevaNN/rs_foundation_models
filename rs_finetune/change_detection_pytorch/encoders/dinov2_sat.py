...
class SSLVisionTransformer(DinoVisionTransformer):
    ...
    def __init__(self,
                ...
                frozen_stages=100,
                 *args, **kwargs):
        super(SSLVisionTransformer, self).__init__(
                img_size=img_size, 
                patch_size=patch_size,
                embed_dim=embed_dim, 
                depth=depth, 
                num_heads=num_heads, *args, **kwargs) 
        ...
        self.adapad = AdaptivePadding(kernel_size=patch_size, stride=patch_size, padding='same')
        if pretrained:
    ...
    def init_weights(self, pretrained):
        if (isinstance(self.init_cfg, dict)
            ...
            if 'pos_embed' in list(state_dict.keys()):
...
class SSLAE(nn.Module):
    def __init__(self, depth=32,  pretrained=None, out_indices=(9, 16, 22, 29, 31), huge=False, classification=False):
        super().__init__()
        if huge == True:
        ...
        if not classification:
            self.neck = MultiLevelNeck(in_channels=[1280, 1280, 1280, 1280], 
                                       out_channels=1280, scales=[4, 2, 1, 0.5])
    ...
    def forward(self, x):
        ...
        outs = [x[i][0] for i in range(len(x))]
...
dinov2_encoders = {
    ...
    "dinov2": {
        "encoder": Dinov2VisionTransformer,
...
if __name__ == "__main__":
    dinov2 = Dinov2VisionTransformer(**dinov2_encoders['dinov2']['params'])
    model = torch.hub.load('facebookresearch/dinov2', 'dinov2_vitb14')
    # test_model = SSLVisionTransformer(**dinov2_encoders['Dinov2-B']['params'])
    new_dict = {'backbone.'+k: v for k, v in model.state_dict().items()}

    dinov2.load_state_dict(new_dict)
    print(dict(dinov2.state_dict())['backbone.pos_embed'].shape)
