...
class TerraFM(nn.Module):
    def __init__(self, img_size=[224], patch_size=16, in_chans=3, num_classes=0, embed_dim=768, depth=12,
                ...
                drop_path_rate=0., norm_layer=nn.LayerNorm, for_cls=False, 
                out_idx=None, out_channels=None):
        super().__init__()
        ...
        self.patch_embed = PatchEmbed(
                img_size=img_size if isinstance(img_size, int) else img_size[0], patch_size=patch_size, in_chans_s1=2, in_chans_s2=12, embed_dim=embed_dim)
    ...
    def prepare_tokens(self, x):
        ...
        # TerraFM expects specific channel counts: 2 for S1 or 12 for S2
        # If we have fewer channels, pad with zeros
        if nc == 2:
            # Sentinel-1 case - keep as is
            pass
        elif nc < 12:
            # Pad to 12 channels for Sentinel-2 path
            padding = torch.zeros(B, 12 - nc, w, h, device=x.device, dtype=x.dtype)
            x = torch.cat([x, padding], dim=1)
        elif nc > 12:
            # Truncate to 12 channels if more than expected
            x = x[:, :12, :, :]
    ...
    def forward(self, x):
        ...
        x = self.prepare_tokens(x)
        B, N, C = x.shape
        h, w = int(math.sqrt(N - 1)), int(math.sqrt(N - 1))  # -1 for CLS token
...
terrafm_encoders = {
    "terrafm-base": {
        "encoder": TerraFM,
        "pretrained_settings": pretrained_settings['TerraFM'],
        "params": {
            "img_size": 224,
            "patch_size": 16,
            "embed_dim": 768,
