...
from .base import Decoder, modules, ClassificationHead, SegmentationHead
...
class FPNBlock(nn.Module):
    def __init__(self, pyramid_channels, skip_channels):
        ...
        self.skip_conv = nn.Sequential(
                ...
                #nn.SyncBatchNorm(pyramid_channels),   # adjust to "SynchronizedBatchNorm2d" if you need.
    ...
    def initialize(self):
        for name, module in self.named_children():
            if name == 'segmentation_head':
                self.initialize_head(module)
            else:
                self.initialize_decoder(module)
