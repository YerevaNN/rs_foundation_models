...
def clip_gradients_fast(model, clip):
    grads = [p.grad for p in model.parameters() if p.grad is not None]
    norms = torch.stack([g.data.norm(2) for g in grads])
    clip_coefs = clip / (norms + 1e-6)
    clip_coefs = torch.clamp(clip_coefs, max=1.0)
...
class FastRandomResizedCrop(transforms.RandomResizedCrop):
    ...
    def __init__(self, size, scale=(0.08, 1.0), ratio=(3.0 / 4.0, 4.0 / 3.0), **kwargs):
        super().__init__(size, scale, ratio, **kwargs)

        # self.log_ratio = torch.log(torch.tensor(self.ratio))
        self.log_ratio = np.log(np.array(self.ratio))

        self.rand_scale = RandomUniformGenerator(self.scale[0], self.scale[1])
        self.rand_ratio = RandomUniformGenerator(self.log_ratio[0], self.log_ratio[1], apply_exp=True)
        self.rand = RandomUniformGenerator(0, 1)
    ...
    def get_params(self, img: Tensor, scale: List[float], ratio: List[float]) -> Tuple[int, int, int, int]:
        ...
        for _ in range(10):
            target_area = area
            target_area = area * self.rand_scale()  # torch.empty(1).uniform_(scale[0], scale[1]).item()
            aspect_ratio = self.rand_ratio()  # torch.exp(torch.empty(1).uniform_(self.log_ratio[0], self.log_ratio[1])).item()
...
class RandomUniformGenerator:
    def __init__(self, min=0, max=1, apply_exp=False, batch_size=5000):
        assert (min <= max)
    ...
    def _data_update(self):
        self._data = np.random.uniform(self.min, self.max, self.batch_size)
        if self.apply_exp:
